  ...
  switch ($act) {
    ...
    default:
      ...
      if ($_SESSION['leveluser'] == 'user') {
        $query  = "SELECT * FROM perbaikan inner join user using(id_user) inner join mesin using(id_mesin)
                    WHERE id_user='$_SESSION[username]' ORDER BY id_perbaikan";
    ...
    case "tambahperbaikan":
      ...
      $query  = "SELECT * FROM akses_lab a inner join prodi b using(id_prodi) inner join lab c using(id_lab) WHERE a.id_prodi='$id_prodi'";
      $tampil = mysqli_query($konek, $query);
      echo $query;
      while ($r = mysqli_fetch_array($tampil)) {
        echo "<option value='$r[id_lab]'>$r[laboratorium]</option>";
      }
      echo "</select>
                              </div>
                            </div>
    ...
    case "penggunaan":
      //GET Kode Otomastis.........
      $query = "select max(id_penggunaan) as maksi from penggunaan";
      $hasil = mysqli_query($konek, $query);
      $data_oto  = mysqli_fetch_array($hasil);
      $kode_user = buatkode($data_oto['maksi'], 'PU', 3);
      $id_prodi = $_SESSION['id_prodi'];
      ...
      echo "<div class='row'>
       <div class='col-xs-12'>
         <div class='widget-box'>
           <div class='widget-header'>
             <h4 class='widget-title'>Form Penggunaan Mesin</h4>
               <div class='widget-toolbar'>
                 <a href='#' data-action='collapse'>
                   <i class='ace-icon fa fa-chevron-up'></i>
                 </a>
           ...
           <div class='widget-body'>
             <div class='widget-main'>
               <form method='POST' action='$aksi?halamane=perbaikan-approval&act=input2'>
                 <div>
                   <label>Kode Penggunaan</label>
                     <div class='row'>
                       <div class='col-xs-8 col-sm-2'>
                         <div class='input-group'>
                           <input type='text' name='id_penggunaan' class='form-control' placeholder='ID penggunaan' required='required' value='$kode_user' readonly='yes'>
                             <span class='input-group-addon'>
                               <i class='fa fa-key bigger-110'></i>
                             </span>
                         </div>
                   ...
                   <label for='id-date-picker-1\'>Tanggal Penggunaan</label>
                     <div class='row'>
                       <div class='col-xs-8 col-sm-6'>
                         <div class='input-group'>
                           <input class='form-control date-picker' id='id-date-picker-2' type='text' name='tgl_buat' data-date-format='yyyy-mm-dd' required='required'/>
      ...
      echo "</select>
                     ...
                     <hr/>
                     <label for='id-date-picker-1'>Keperluan</label>
                     <div class='row'>
                       <div class='col-sm-6'>
                         <input class='form-control' type='text' name='judul' required='required'/>
                       </div>
    ...
    case "editperbaikan":
      ...
      echo " <div class='row'>
            <div class='col-xs-12'>
              ...
              <div class='col-xs-8 col-sm-6'>
              <div class='form-group'>
              <label for='form-field-select-3'>Pilih Pr
